...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        verification_url = f"https://8140-2a02-8428-2979-201-acee-f626-9af9-cbc.ngrok-free.app/?userId={user_id}"
...
async def display_pending_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not pending_messages:
        message_text = 'You have no pending messages.'
        if update.callback_query:
            try:
                await update.callback_query.message.edit_text(
                    message_text,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh Inbox", callback_data="check_inbox")]])
                )
            except telegram.error.BadRequest as e:
                if "Message is not modified" in str(e):
                    # Message hasn't changed, just ignore the error
                    pass
                else:
                    raise e
        else:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    message_text = "Select a message to read:"
    ...
    if question_index < len(user_questions[user_id]):
        ...
        if update.callback_query:
            await message.edit_text(
                completion_message,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 My Inbox", callback_data="check_inbox")]])
            )
        else:
...
async def handle_message_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if selected_message:
            ...
            waiting_message = await query.message.reply_text("Please read the message above carefully. The questions will start in 15 seconds...")
            await asyncio.sleep(15)  # Give user 5 seconds to read
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(display_pending_messages, pattern="^check_inbox$"))
